<!DOCTYPE html>
<html>
    <head>
        <title>Friends</title>
        <link rel="stylesheet" type="text/css" href="/static/css/common.css"></link>
        <link rel="stylesheet" type="text/css" href="/static/css/friends.css">
        <script src="http://use.edgefonts.net/indie-flower:n4:all.js"></script>
    </head>
    </head>
    
    <body>
        <div id="header">
            <h1>StoryTime Friends</h1>
            <a href="" id="notifications">Notifications</a>
        </div>         
            
    
        <ul id="navbar">
            <li class="page"><a href=".." class="plinks">Home</a></li>
            <li class="page"><a href="" class="plinks">Friends</a></li>
            <li class="page"><a href="settings" class="plinks">Settings</a></li>
            <li class="page"><a href="help" class="plinks">Help</a></li>
        </ul>
        
        <div id="FriendsList" class="list">
            <h2>Friends</h2>
            <ul id="flist">
                <li class="friend">Ali Dehghan</li>
                <li class="friend">Brandon Mabey</li>
                <li class="friend">Courtnay Low</li>
                <li class="friend">Sarah Warnock</li>
                <li class="friend">Shaquille Davis</li>
            </ul>
        </div>
    </body>
</html>